...
mopet <- function (M, env, method = c('lm'), cor.coef = c('pearson'), dependence = "M ~ env", permutations = 499, test = "modified", parallel = NULL)
{
   ...
   result <- list (real.summaries = real.summaries, coefs = coefs, stat = res.real.stat, orig.P = orig.P, perm.P = perm.P, modif.P = modif.P, seq.P = seq.P, permutations = permutations)
   class (result) <- 'mopet'
   return (result)
}
...
print.mopet <- function (x, digits = max(3, getOption("digits") - 3), ...)
{
  ...
  if (!is.null (x$seq.P)) mat.seq.P <- cbind (seq.P = format.pval (x$seq.P, digits = digits), symnum.pval (x$seq.P))
  print.default (cbind (mat.coef, mat.stat, if (!is.null (x$orig.P)) mat.orig.P, if (!is.null (x$perm.P)) mat.perm.P, if (!is.null (x$modif.P)) mat.modif.P, if (!is.null (x$seq.P)) mat.seq.P), quote = F, right = T, ...)
}
