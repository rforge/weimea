...
#' @param x object of the class "mopet", generated by function \code{'mopet'}.
...
print.mopet <- function (x, digits = max(3, getOption("digits") - 3))
{
  ...
  mat.coef <- matrix (format (unlist (x$coefs), digits = digits), byrow = T, nrow = length (x$coefs))
  colnames (mat.coef) <- names(x$coefs[[1]])
  mat.stat <- matrix (format (unlist (x$stat), digits = digits))
  colnames (mat.stat) <- names (x$stat[[1]])
  if (!is.null (x$orig.P)) mat.orig.P <- cbind (orig.P = format.pval (x$orig.P, digits = digits), symnum.pval (x$orig.P))
  if (!is.null (x$perm.P)) mat.perm.P <- cbind (perm.P = format.pval (x$perm.P, digits = digits), symnum.pval (x$perm.P))
  if (!is.null (x$modif.P)) mat.modif.P <- cbind (modif.P = format.pval (x$modif.P, digits = digits), symnum.pval (x$modif.P))
  print.default (cbind (mat.coef, mat.stat, if (!is.null (x$orig.P)) mat.orig.P, if (!is.null (x$perm.P)) mat.perm.P, if (!is.null (x$modif.P)) mat.modif.P), quote = F, right = T)
}

#' @export
#' @rdname mopet
summary.mopet <- function (x)
{
  len <- length (x$real.summaries)
  cat ('\nSummary of mopet function:\n\n')
  for (i in seq (1, len))
  {
    cat ('\nOriginal result for variable', names (x$real.summaries)[i], ':\n')
    print (x$real.summaries[[i]])
    cat ('\n------------------------------------------------')
    if (!is.null (x$perm.P[i])) cat ('\nStandard permutation test: P = ', x$perm.P[i])
    if (!is.null (x$modif.P[i]))cat ('\nModified permutation test: P = ', x$modif.P[i])
    cat ('\nPermutation results based on', x$permutations, 'permutations')
    cat ('\n************************************************\n')
  }
