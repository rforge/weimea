...
#' 
#' For the purpose of the current dataset, species in shrub and tree layer have been merged, juveniles removed and nomenclature have been modified according to Kubat et al. (2002). Dataset has two parts: with all (tree, shrub and herb) species (\code{vltava$spe, $ell, $env} etc.) and with subset of only herb species (\code{vltava$herbs$spe, $ell, $traits etc.}). While Ellenberg indicator values are provided for both all and only herb species subset, plant functional traits are only for subset of herb species (it would perhaps not be meaningful to compare e.g. SLA or plant height for trees and herbs). Some species indicator values and trait values remain unasigned, because they are not conteined in either Ellenberg or LEDA database. Exception is SLA value for Impatiens parviflora, which in LEDA database has assigned value 148.01 mm2/mg, which seems as outlier and have been temporarily removed.
#' 
#' 
...
#' \item COVERE3 Estimated cover of tree layer [percentage]
#' \item COVERE2 Estimated cover of shrub layer [percentage]
#' \item COVERE1 Estimated cover of herb layer [percentage]
#' \item COVERE32 Sum of estimated cover of tree and shrub layer [percentage] (see Details for calculation)
#' }
#' @usage data (vltava)
#'  @format
